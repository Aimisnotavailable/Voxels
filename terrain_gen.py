...
def get_height(x, z):
    # amplitude
    # cast inputs to float64 to keep types consistent
    dx = float(x) - CENTER_XZ
    dz = float(z) - CENTER_XZ
    c = 20.0

    # 3D Euclidean distance (nopython-friendly)
    dist = math.sqrt(dx*dx + dz*dz + c*c)

    # amplitude (use float arithmetic)
    island = 1.0 / ((0.0025 * dist) ** 20.0 + 0.0001)
...
def place_tree(voxels, x, y, z, voxel_id):
    ...
    for n, iy in enumerate(range(TREE_H_HEIGHT, TREE_HEIGHT - 1)):
        k = iy % 2
        rng = int(random() * 2)
        for ix in range(-TREE_H_WIDTH + m * rng, TREE_H_WIDTH - m * rng):
